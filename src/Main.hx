...
class Main extends Sprite 
{
	...
	public function new() 
	{
		...
		var smallTextView = new TextView("Текст", contentWidth, contentHeight);
		...
		var buttonView = new ButtonView(4, 4, contentWidth, contentHeight);
		
		navigator.dataProvider = new ArrayCollection([
			TabItem.withDisplayObject("Небольшой текст", smallTextView),
			TabItem.withDisplayObject("Большой текст", bigTextView),
			TabItem.withDisplayObject("Кнопки", buttonView)
		]);
